...
def optimize_charging_strategy(charging_stations, vehicle_params, charging_curve):
    ...
    optimal_strategy, total_time = reconstruct_optimal_path(dp_table, charging_stations)
        ...
        if current_soc not in dp_table[current_stage_index]:
            ...
            for soc, data in dp_table[current_stage_index].items():
                if data['time'] < min_time:
                    min_time = data['time']
                    current_soc = soc
            print(f"Warning: Expected SoC {current_soc} not found at stage {current_stage_index}, using the minimum time state instead.")
